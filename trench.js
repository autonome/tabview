...
//
// Parameters:
//   element - the DOM element for Item from which the trench is projected
...
Trench.prototype = {
  ...
  // Computes and sets the <activeRange> for the trench, based on the <Items> around.
  // This makes it so trenches' active ranges don't extend through other items.
  calculateActiveRange: function Trench_calculateActiveRange() {
    ...
    items.forEach(function(item) {
      if (item.isDragging) // floating items don't block trenches
        return;
      if (trench.el == item.container) // items don't block their own trenches
        return;
